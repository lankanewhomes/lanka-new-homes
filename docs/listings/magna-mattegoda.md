# Magna

- **Slug:** `magna-mattegoda` · **Developer:** Prime Lands · **Published:** yes
- **Source:** https://www.primelands.lk/house/MAGNA-MATTEGODA/en
- **Location:** Mattegoda, Mattegoda, Colombo District, Western Province
- **Type / status:** House · Now Selling · units — · floors — · beds — · expected handover —
- **Payment plan badge:** — · **Deposit terms:** —
- **Brochure on listing:** https://media.lankanewhomes.com/projects/magna-mattegoda/brochure/magna-mattegoda_brochure.pdf
- **Live:** /projects/magna-mattegoda

## Floor plans

| Plan | Beds | Baths | SqFt | Perches | 2D | m² | 3D | Downloads | Availability | Available floors |
|---|---|---|---|---|---|---|---|---|---|---|
| TYPE A | 2 | 1 | 742 | — | ✅ | — | — | — | Available | — |
| TYPE B | 2 | 1 | 774 | — | ✅ | — | — | — | Available | — |
| TYPE C | 2 | 1 | 848 | — | ✅ | — | — | — | Available | — |

## Media (Cloudflare R2, media.lankanewhomes.com)

- Hero: https://media.lankanewhomes.com/projects/magna-mattegoda/gallery/magna-mattegoda_exterior-01.jpg
- Property photos (gallery/): 1 · amenity photos (amenities/): 0 · road map: yes
- Construction updates (dated photos): 0

## Data captured

- Amenities: —
- Key Features groups: —
- Nearby places: 4
- Contact: — · — · —
- Project verification checklist: not yet

## Sources & brochures

- magna-mattegoda_brochure.pdf (attached)

## Notes, assumptions & gaps

- All 3 types (A–C) confirmed — each single-storey with its own printed total: A 742, B 774, C 848 sqft, all 2bd/1ba.
- 4 nearby transport points added (transport hub, two bus routes, highway entrance, all with stated minutes rather than km).
- No price or payment-plan terms published ("Contact for pricing").

_Generated 2026-09-09 from the live Payload record._
